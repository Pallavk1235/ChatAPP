            ...
            $.ajax({
                ...
                success: function(seen_message) {
                    ...
                    for (var i = 0; i < seen_message.length; i++) {
                        if (seen_message[i].user_from == sender_user_id) {
                            var my_msg =
                        ...
                        } else if (seen_message[i].user_from == logged_user) {
                            var my_msg =
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('#contacts tr').each(function() {
                        sender_id = $('tr#ClickableRow').find('td').eq(2).text();
                        for (var i = 0; i < data.length; i++) {
                            var new_sender = parseInt(sender_id);
                            if (new_sender == data[i].user_from) {
                                var count = data[i].message_count;
                                $("#count_" + new_sender).html(
                                    '<span class="right badge badge-danger">' + count +
                                    '</span>');
                            }
                        }
                    })
